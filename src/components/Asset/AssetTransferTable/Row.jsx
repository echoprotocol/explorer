...
import { SSR_BLOCK_INFORMATION_PATH, SSR_ASSET_PATH } from '../../../constants/RouterConstants';
		...
		<tr onClick={onClick} className={cn('view')}>
			...
			<td className="account">
				<Link
					...
					as={URLHelper.createAccountUrl(from && from.id)}
				>
			...
			<td className="account">
				<Link
					href={SSR_BLOCK_INFORMATION_PATH}
					as={URLHelper.createAccountUrl(to && to.id)}
				>
					<a className="td-in link">
			...
			<td className="amount">
				{fee &&
					<Link
						href={SSR_ASSET_PATH}
						as={URLHelper.createAssetUrl(fee.asset_id)}
					>
						<a className="td-in link">
							<div className="td-in">
